        ...
        <div className="min-h-screen bg-gray-100 p-8">
            ...
            <div className="mx-auto w-auto mb-6">
                <div className="flex flex-wrap justify-center">
                    <button
                        className={`flex-shrink-0 w-full sm:w-auto px-4 py-2 rounded ${filter === '' ? 'bg-blue-500 text-white' : 'bg-white text-blue-500 border border-blue-500'}`}
                        onClick={() => setFilter('')}
                    >
                        All ({words.length})
                    </button>
                    {partsOfSpeech.map((part, index) => (
                        <button
                            key={index}
                            className={`flex-shrink-0 w-full sm:w-auto p-1 text-xs rounded ${filter === part ? 'bg-blue-500 text-white' : 'bg-white text-blue-500 border border-blue-500'}`}
                            onClick={() => setFilter(part)}
                        >
                            {part.charAt(0).toUpperCase() + part.slice(1)} ({words.filter(word => word.partOfSpeech === part).length})
                        </button>
                    ))}
                </div>
            </div>
            <div className="flex flex-wrap justify-center gap-6">
                {filteredData.slice(0, visibleCount).map((item, index) => (
                    <div key={index} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 rounded overflow-hidden shadow-lg bg-white transform transition-transform hover:scale-105">
                        <div className="px-6 py-4">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-2xl text-blue-700 mr-2">{item.word}</span>
                                <span className="text-gray-500 text-sm">{item.partOfSpeech.charAt(0).toUpperCase() + item.partOfSpeech.slice(1)}</span>
                            </div>
                            <p className="text-gray-700 text-base mb-3">{item.definition}</p>
                            <p className="text-gray-700 text-base">
                                <svg className="inline-block w-4 h-4 mr-2 fill-current text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
